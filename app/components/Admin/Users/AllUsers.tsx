...
// const AllUsers: FC<Props> = ({ isTeam }) => {
//   const { theme } = useTheme(); // removed unused setTheme
//   const [active, setActive] = useState(false);
//   const [email, setEmail] = useState("");
...
//       const errorMessage = updateError as { data: { message: string } };
//       toast.error(errorMessage.data.message);
//     }
//     if (isSuccess) {
//       refetch();
...
//       toast.error(errorMessage.data.message);
//     }
//   }, [updateError, isSuccess, deleteSuccess, deleteError, refetch]); // added refetch dependency
...
//   const usersData = isTeam
//     ? data?.users.filter((item: User) => item.role === "admin")
//     : data?.users;
...
// This matches your backend data shape
interface ApiUser {
  _id: string;
  name: string;
  email: string;
...
// Shape expected by the DataGrid
interface User {
  id: string;
  name: string;
  email: string;
...
const AllUsers: FC<Props> = ({ isTeam }) => {
  ...
  // ✅ Define columns properly with type
  ...
  // ✅ Safely convert API users into DataGrid-friendly rows
  const apiUsers: ApiUser[] = data?.users || [];

  const usersData = isTeam
    ? apiUsers.filter((item) => item.role === "admin")
    : apiUsers;

  const rows: User[] = usersData.map((item) => ({
    id: item._id, // ✅ Map backend _id → DataGrid id
    name: item.name,
    email: item.email,
    role: item.role,
    courses: item.courses || [],
